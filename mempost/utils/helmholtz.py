"""2D Helmholtz equation solver with PML absorbing boundaries.

Self-contained scipy-based solver for frequency-domain acoustic wave
propagation.  The forward map F(m) = P A(m)^{-1} b is nonlinear in the
squared-slowness m --- exactly the setting needed to test Eq. 6.

Physics:
    A(m) u = b   where   A = omega^2 diag(m) + Delta_PML
    F(m)   = P u   (extract at receiver locations)

PML:
    Quadratic ramp sigma(d) = sigma_max (d / L_pml)^2
    Complex stretch s(x) = 1 + i sigma(x) / omega
    Modified Laplacian discretized with 5-point stencil.
"""
...
class HelmholtzSolver:
    ...
    def _pml_profile(self, n_total, npml, omega):
        ...
        sx = 1.0 + 1j * sigma / omega
        sx_half = 0.5 * (sx[:-1] + sx[1:])
        return sx, sx_half
    ...
    def _build_laplacian(self, omega):
        ...
        c_xp = np.zeros(nx_t, dtype=complex)
        c_xm = np.zeros(nx_t, dtype=complex)
        for i in range(nx_t - 1):
            c_xp[i] = 1.0 / (sx[i] * sx_half[i])
        for i in range(1, nx_t):
            c_xm[i] = 1.0 / (sx[i] * sx_half[i - 1])

        c_zp = np.zeros(nz_t, dtype=complex)
        c_zm = np.zeros(nz_t, dtype=complex)
    ...
    def _build_system_matrix(self, m_padded, omega):
        ...
        mass = sp.diags(omega**2 * m_padded.ravel(), format="csc")
        return L + mass

    def _pad_model(self, m):
        """Pad interior squared-slowness field with edge values for PML."""
        npml = self.npml
        m_padded = np.zeros((self.nx_total, self.nz_total), dtype=complex)
    ...
    def forward(self, m, omega, src_positions, rec_positions):
        ...
        n_rec = len(rec_positions)
        data = np.zeros((n_src, n_rec), dtype=complex)
    ...
    def adjoint_gradient(self, m, omega, src_positions, rec_positions,
        ...
        # LU factorize A and conj(A).
        lu_fwd = spla.splu(A)
        lu_adj = spla.splu(A.conj())

        npml = self.npml
        grad_full = np.zeros(self.N, dtype=float)
...
def make_src_rec(nx, nz, n_src=5, n_rec=40):
    """Create surface source and bottom receiver positions.

    Sources: evenly spaced along top (iz=0).
    Receivers: evenly spaced along bottom (iz=nz-1).
    """
    src_ix = np.linspace(0, nx - 1, n_src, dtype=int)
    src_positions = [(int(ix), 0) for ix in src_ix]

    rec_ix = np.linspace(0, nx - 1, n_rec, dtype=int)
    rec_positions = [(int(ix), nz - 1) for ix in rec_ix]
